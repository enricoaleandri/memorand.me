 ...
 * @author  <Daniele Pace>  <daniele.a.pace@avanade.com>
...
/**
 * @desc passa ad un branch
 * @param branchRepo {string} - il nome del branch
 * @returns {Promise}
 */
var passToBranchProject = function(branchRepo) {
	return new Promise(function(resolve,reject){

		var cmdCommand = 'git checkout ' + branchRepo;
		var chdir = options.paths.coreRepository.match(/[A-z\-]+$/);
		var currDir = process.cwd();

		console.log('cd '+chdir);
		try {
			process.chdir('./'+chdir);
			console.log('New directory: ' + process.cwd());
			exec(cmdCommand, function (err, out, code) {
			...
			console.log('<passToBranchProject>: '+err);
			if(currDir != process.cwd()){
				process.chdir('..');
				console.log('New directory: ' + process.cwd());
			}
			reject(err);
...
if ( typeof options.paths.branchCore === 'undefined')
	options.paths.branchCore = "master";
if ( typeof options.paths.branchCommon === 'undefined')
	options.paths.branchCommon = "master";
...
process.argv.forEach(function (val, index, array) {
	...
	if(val.indexOf("--branch:") != -1){
		arg["branchOfRepo"]=val.replace("--branch:", "");
	}
...
cloneProject(options.paths.coreRepository)
	.then(function(repository) {
		...
		if (arg["branchOfRepo"] && (arg["branchOfRepo"] != "" || arg["branchOfRepo"] != "master")) {
			return passToBranchProject(arg["branchOfRepo"]);
		}
	})
	.then(function(res) {
...
// cancelliamo la directory "./grunt_config"
		return deletePath("./grunt_config");
	},
	function(reason){
		console.log('Warning: you must delete cloned repo');
		throw reason;
	})
	.then(function() {
...
// copiamo la directory "./grunt_config"
		return copyPath("./" + options.paths.corePath + "/grunt_config", "./grunt_config");
	})
	.then(function () {
...
// copiamo gruntfile.js
		return copyPath("./" + options.paths.corePath + "/_Gruntfile.js", "Gruntfile.js");
	})
	.then(function() {
		console.log("gruntfile succesfully copied");

// eliminiamo la directory  az-direct-IT-JS-core
		return deletePath("./az-direct-IT-JS-core");
	})
	.then(function () {
	...
	.catch(function(reason){
		console.log("\n\nOh no! :-( The script exites for this error: \n\n" + reason +"\n \nprobably Alessandro will punches us: don't tell him this! ;-)");

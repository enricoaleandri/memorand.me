...
module.exports = function(options, grunt) {
	...
	function getJsFiles(){
		return jsFiles = getFiles(paths.dist + "/app/js");
	}
	function getCssFiles(){
		return getFiles(paths.app + "/css").map(function(file){
			var fileName = file.replace("/css/", "/css/").split(".css")[0];
	...
	return {
		...
		'dist': {
			'options': {
				'data': {
					...
					'scripts':getJsLibs().map(function(lib){
						...
						var pieces = appJs.split("/");
						return "app/js/" + pieces[pieces.length - 1];
					})).toString(),
					'ie8_styles': '',
					'ie8_scripts': '',
					'type': '2'
			...
			'files': {
				'memorand.me/index.properties': ['index.properties']
			}
		}

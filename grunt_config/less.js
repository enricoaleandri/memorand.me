    ...
    function getFiles(dir, files_, dirName, isMin) {
        ...
        for (var i in files) {
            ...
            if (fs.statSync(name).isDirectory()) {
                getFiles(name, files_, files[i], isMin);
            }
            else {
                var ourDirName = dirName.split("/");
                var extDir = ourDirName[ourDirName.length - 1].length ? ourDirName[ourDirName.length - 1] + "/" + noExtName : noExtName;
                files_[(isMin ? paths.dist : paths.dev) + "/app/css/" + extDir + (isMin ? ".min" : "") + ".css"] = name;
            }
        }
        return files_;
    }
